...
namespace TableCore.Lobby
{
    ...
    public partial class PlayerCustomizationHud : Control
    {
        ...
        private void SyncFromProfile()
        {
            ...
            ConfigurePlaceholderBehavior();
            UpdateNameLabelFromModel();
        ...
        private void ConfigurePlaceholderBehavior()
        {
            if (_model is null)
            {
                ...
                _autoClearOnType = false;
                return;
            }

            _placeholderEnabled = IsAutoGeneratedDisplayName(_model.BaselineName);
            _autoClearOnType = _placeholderEnabled && string.Equals(_model.DisplayName, _model.BaselineName, StringComparison.Ordinal);
        }

        private void UpdateNameLabelFromModel()
        {
            if (_model is null)
            {
                return;
            ...
            var displayName = _model.DisplayName;
            if (_placeholderEnabled && string.IsNullOrWhiteSpace(displayName))
            {
            ...
            _autoClearOnType = _placeholderEnabled && string.Equals(displayName, _model.BaselineName, StringComparison.Ordinal);
        }

        private void OnKeyboardKeyPressed(string key)
        {
            if (!_placeholderEnabled || !_autoClearOnType)
            {
                return;
            }

            if (!ShouldClearPlaceholderOnKey(key))
            {
                return;
            }
        ...
        internal static bool IsAutoGeneratedDisplayName(string? value)
        {
        ...
        internal static bool ShouldClearPlaceholderOnKey(string? key)
        {
            ...
            return !string.Equals(key, FloatingKeyboardSpecialKeys.Enter, StringComparison.Ordinal);
        }
    }
}
